...
import com.example.queue_calculator.schemas.validateMM1InfiniteProps
...
@Composable
fun MM1_Infinite_Screen(navController: NavController) {
    QueueCalculatorTheme {
        Scaffold(
            ...
            MM1_Infinite(innerPadding)
        }
    }
}

@Composable
fun MM1_Infinite(innerPadding: PaddingValues) {
    ...
    fun submit() {
        ...
        val calculator: ICalculator = MM1Infinite()
        ...
        val result = validateMM1InfiniteProps(props)
        when (result.isValid) {
            true -> {
...
@Composable
fun MM1_Infinite_Preview() {
    MM1_Infinite_Screen(rememberNavController())
}