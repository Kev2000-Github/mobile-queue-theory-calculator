...
import com.example.queue_calculator.schemas.validateMMCFiniteProps
...
val inputsDataMMCFinite = listOf(
    mapOf(
    ...
    mapOf(
        "label" to "Capacidad maxima",
        "key" to "N"
    ),
)

@Composable
fun MMC_Finite_Screen(navController: NavController) {
    QueueCalculatorTheme {
        Scaffold(
            ...
            MMC_Finite(innerPadding)
        }
    }
}

@Composable
fun MMC_Finite(innerPadding: PaddingValues) {
    ...
    fun submit() {
        ...
        val calculator: ICalculator = MMCFinite()
        ...
        val maxCapacity = entries.find { it.first == "N" }!!.second
        ...
        val result = validateMMCFiniteProps(props)
        when (result.isValid) {
            true -> {
    ...
    if (queueResult != null) {
        ...
        Column(modifier = Modifier
            ...
            LazyColumn(
                ...
                items(inputsDataMMCFinite) { data ->
                    InputField(
            ...
            KeyPadGeneral(
                keypadData = when {
                    listOf("N", "maxServers").contains(entries[activeInput].first) -> NoCommaKeyPad
                    else -> CommonKeyPads
                },
...
@Composable
fun MMC_Finite_Preview() {
    MMC_Finite_Screen(rememberNavController())
}